...
func ListStores(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		isBlockedRaw := strings.TrimSpace(c.Query("is_blocked"))
		...
		if roleStr, ok := roleVal.(string); ok && roleStr == "superadmin" {
			...
			q = q.Joins("JOIN store_users su ON su.store_id = stores.id").Where("su.user_id = ?", uid)
		}

		if country != "" && country != "全部" && strings.ToLower(country) != "all" {
			q = q.Where("stores.country = ?", country)
		}

		switch status {
		case "enabled":
			q = q.Where("stores.is_blocked = ?", false)
		case "disabled":
			q = q.Where("stores.is_blocked = ?", true)
		}

		if isBlockedRaw != "" {
			isBlockedVal, err := strconv.ParseBool(strings.ToLower(isBlockedRaw))
			if err != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": "is_blocked 参数无效，应为 true/false"})
				return
			}
			q = q.Where("stores.is_blocked = ?", isBlockedVal)
		}

		if err := q.Order("stores.name asc").Find(&list).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		items := make([]storeListItem, 0, len(list))
		storeIDs := make([]uint, 0, len(list))
		...
		if len(storeIDs) > 0 {
			...
			if err := db.Table("store_users su").
				Select("su.store_id, su.user_id, users.username").
				Joins("JOIN users ON users.id = su.user_id").
			...
			byStore := map[uint][]storeUserRow{}
			for _, row := range rows {
				byStore[row.StoreID] = append(byStore[row.StoreID], row)
			}
			for i := range items {
				for _, row := range byStore[items[i].ID] {
					items[i].UserIDs = append(items[i].UserIDs, row.UserID)
					if strings.TrimSpace(row.Username) != "" {
						items[i].UserNames = append(items[i].UserNames, row.Username)
					}
				}
			}
		}
...
func HomeDashboard(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		// 默认统计当前月，允许通过 year/month 查询其他月份
		now := time.Now()
		targetYear := now.Year()
		...
		if yearStr != "" && monthStr != "" {
			...
			m, err2 := strconv.Atoi(monthStr)
			if err1 != nil || err2 != nil || m < 1 || m > 12 {
				c.JSON(http.StatusBadRequest, gin.H{"error": "月份参数格式无效"})
				return
			}
			targetYear = y
			targetMonth = m
		...
		monthStart := time.Date(targetYear, time.Month(targetMonth), 1, 0, 0, 0, 0, time.Local)
		...
		type monthAgg struct {
			...
			ReturnLoss  float64 `gorm:"column:return_loss"`
		...
		// 1.1 当月退货利润亏损总和（按退货业务日期聚合）
		type returnLossAgg struct {
			ReturnLoss float64 `gorm:"column:return_loss"`
		}
		var ragg returnLossAgg

		rq := db.Model(&models.ReturnRecord{}).
			Select("COALESCE(SUM(loss_amount), 0) AS return_loss").
			Where("return_date >= ? AND return_date <= ?", startDate, endDate)
		if roleStr != "superadmin" {
			uid, ok := userIDVal.(uint)
			if !ok || uid == 0 {
				c.JSON(http.StatusUnauthorized, gin.H{"error": "未获取到用户信息"})
			...
			rq = rq.Where("user_id = ?", uid)
		}
		if err := rq.Scan(&ragg).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		agg.ReturnLoss = ragg.ReturnLoss
		// 首页利润改为净利润口径：结算利润 - 退货亏损
		agg.Profit = agg.Profit - agg.ReturnLoss
		...
		// 3. 当月商品退货情况：按商品名称汇总退货数量
		type returnProductAgg struct {
			ProductName string `json:"product_name" gorm:"column:product_name"`
			Quantity    int    `json:"quantity" gorm:"column:qty"`
		}
		var returnProducts []returnProductAgg

		rp := db.Model(&models.ReturnRecord{}).
		...
		if roleStr != "superadmin" {
			...
			rp = rp.Where("user_id = ?", uid)
		}
		if err := rp.Group("product_name").Order("qty DESC").Limit(50).Scan(&returnProducts).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}

		// 4. 店铺数量统计：总店铺数及按国家分组数量（仅统计未封禁店铺）
		...
		c.JSON(http.StatusOK, gin.H{
			...
			"monthly_metrics": gin.H{
				...
				"return_loss": agg.ReturnLoss,
				"profit":      agg.Profit,
			},
			"product_sales":   products,
			"product_returns": returnProducts,
			"store_stats": gin.H{
				"total":      totalStores,
